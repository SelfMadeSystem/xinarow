...
export type RoomOptions = {
    ...
    playerTurns: number; // also 1 for most games
    gravity: boolean;
    gridType: GridType;
    teamOrder: TeamOrder;
    width: number;
...
export type ClientPackets = {
    ...
    uid: [string];
    // Asks to join a room.
    join: [uid: string, username: string, room: string];
    // Creates a room
    create: [
        uid: string,
        username: string,
...
export type ServerPackets = {
    ...
    joinAccept: [options: RoomOptions];
    ...
    // Chat message.
    playerChat: [username: string, message: string];
    // Tells the client the game is over because someone won.
    gameWon: [winner: PlayerColor, winningLines: [x: number, y: number][][]];
...
export function emitPacket<Name extends ClientPacketNames | ServerPacketNames>(
    emitter: {
...
export function onPacket<Name extends ClientPacketNames | ServerPacketNames>(
    emitter: {
        on: (arg0: string, arg1: (...arg1: any) => void) => any;
    },
    name: Name,
    listener: (...arg1: (ClientPackets & ServerPackets)[Name]) => void
) {
    emitter.on(name, listener);
}

export function offPacket<Name extends ClientPacketNames | ServerPacketNames>(
    emitter: {
